...
export default function ManagePage() {
    ...
    const [manageProductsList, setManageProductsList] = useState<Product[]>([]);
    const [hppTemplates, setHppTemplates] = useState<HppTemplate[]>([]);
    ...
    const fetchProducts = async () => {
        ...
        if (data) {
            setManageProductsList(data.map((p: any) => ({
                id: p.id,
                name: p.name,
                category: p.category,
                variant: p.variant || '',
                ...
                hppTemplateId: p.hpp_template_id,
    ...
    const fetchHppTemplates = async () => {
        ...
        if (data) {
            setHppTemplates(data.map((h: any) => ({
                id: h.id,
                name: h.name,
                total: h.total
            })));
    ...
    useEffect(() => {
        ...
        const channelProducts = supabase
            .channel('realtime-manage-products')
    ...
    const handleAddProduct = async (e: React.FormEvent) => {
        e.preventDefault();
        const finalPrice = formVariant === 'Manual Brew' ? 0 : parseInt(formPrice || '0');
        if (!formName || (formVariant !== 'Manual Brew' && !formPrice) || !formStock) {
            return showStatus('ERROR', 'Peringatan!', 'Isi Nama, Harga, dan Stok dengan lengkap!');
        }

        let hppTotal = 0;
        if (formHppId) {
            const selectedHpp = hppTemplates.find(h => h.id === parseInt(formHppId));
            if (selectedHpp) hppTotal = selectedHpp.total;
        }
        ...
        try {
            const { error } = await supabase
                ...
                .insert({
                    ...
                    hpp_template_id: formHppId ? parseInt(formHppId) : null,
                    hpp_total: hppTotal
                });

            if (error) throw error;

            showStatus('SUCCESS', 'Berhasil!', `Menu "${formName}" sukses ditambahkan ke master database.`);
            setFormName(''); setFormPrice(''); setFormStock('50'); setFormVariant(''); setFormHppId('');
            setIsFormOpen(false);
            fetchProducts();
        } catch (error) {
            showStatus('ERROR', 'Gagal!', 'Terjadi kesalahan saat menyimpan menu ke Supabase.');
        }
    };
    ...
    const startEdit = (product: Product) => {
        ...
        setEditForm({
            ...
            hppId: product.hppTemplateId?.toString() || ''
        });
    };

    const saveEdit = async () => {
        const finalEditPrice = editForm.variant === 'Manual Brew' ? 0 : parseInt(editForm.price || '0');
        if (!editForm.name || (editForm.variant !== 'Manual Brew' && !editForm.price)) {
            return showStatus('ERROR', 'Peringatan!', 'Form edit menu tidak boleh ada yang kosong!');
        }

        let hppTotal = 0;
        if (editForm.hppId) {
            const selectedHpp = hppTemplates.find(h => h.id === parseInt(editForm.hppId));
            if (selectedHpp) hppTotal = selectedHpp.total;
        }
        ...
        try {
            const { error } = await supabase
                ...
                .update({
                    ...
                    hpp_template_id: editForm.hppId ? parseInt(editForm.hppId) : null,
                    hpp_total: hppTotal
                })
                .eq('id', editId);
            ...
            showStatus('SUCCESS', 'Berhasil!', `Menu "${editForm.name}" berhasil diperbarui.`);
            setEditId(null);
            fetchProducts();
        } catch (error) {
            showStatus('ERROR', 'Gagal!', 'Gagal memperbarui master menu.');
        }
    };
    ...
    const handleDeleteProduct = async (id: number, pName: string) => {
        if (confirm(`Hapus menu "${pName}" dari daftar aktif?`)) {
    ...
    if (!isMounted) {
        return (
            <div className="h-screen w-screen bg-zinc-950 flex items-center justify-center">
                <p className="text-amber-500 font-black tracking-[0.3em] uppercase text-xs">Memuat Manajemen Menu...</p>
            </div>
        );
    }
            ...
            <div className="flex-1 overflow-hidden flex flex-col min-w-0">
                <div className={`flex items-center justify-between px-4 sm:px-8 py-4 border-b flex-shrink-0 ${d ? 'border-zinc-800 bg-zinc-900' : 'border-zinc-200 bg-white'}`}>
                    <div>
                        ...
                        <p className={`text-[9px] font-black uppercase tracking-widest ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>{manageProductsList.length} menu aktif</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto overflow-x-auto">
                                            ...
                                            <>
                                                ...
                                                <td className="px-4 py-3.5 text-right">
                                                    <div className="flex items-center justify-end gap-1 md:gap-1.5">
                                                        ...
                                                        <button onClick={() => handleDeleteProduct(p.id, p.name)} className={`p-1.5 md:p-2 rounded-lg transition-all ${d ? 'text-red-400 bg-red-500/10 hover:bg-red-500/20' : 'text-red-500 bg-red-50 hover:bg-red-100'}`}><Trash2 size={12} className="md:w-3.5 md:h-3.5" /></button>
                                                    </div>
                                                </td>
                                            </>
                                        )}
                    ...
                    {manageProductsList.length === 0 && (
                        <div className={`flex flex-col items-center justify-center py-20 md:py-24 ${d ? 'text-zinc-700' : 'text-zinc-300'}`}>
                            ...
                            <p className="font-black text-[10px] md:text-xs uppercase tracking-widest">Belum ada menu aktif</p>
                        </div>
                    )}
                </div>
            </div>
