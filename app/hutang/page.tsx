    ...
    const batalinHutang = async (h: any) => {
        ...
        try {
            for (const item of h.items) {
                ...
                if (p) {
                    await db.products.update(item.productId, { stock: p.stock + item.quantity });
                }
            }
        ...
        <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-300 ${d ? 'bg-zinc-950 text-white' : 'bg-zinc-100 text-zinc-900'}`}>
            ...
            {payModal && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className={`rounded-2xl p-6 w-full max-w-sm shadow-2xl border ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                        <div className="flex justify-between items-center mb-5">
                            <h3 className="text-sm font-black uppercase tracking-widest text-amber-500">Pilih Pembayaran Lunas</h3>
                            <button onClick={() => setPayModal(null)} className={`p-1.5 rounded-lg ${d ? 'hover:bg-zinc-800' : 'hover:bg-zinc-100'}`}><X size={16} /></button>
                        </div>
                        <p className="text-xs mb-4">Lunasin hutang <strong>{payModal.customerName}</strong> sebesar <strong className="text-amber-500">Rp {payModal.totalAmount.toLocaleString('id-ID')}</strong></p>

                        <div className="flex gap-2 mb-4">
                            <button onClick={() => setActiveMethod('CASH')} className={`flex-1 py-2.5 rounded-xl font-black text-[10px] tracking-wider ${activeMethod === 'CASH' ? 'bg-amber-500 text-black' : d ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-500'}`}>CASH</button>
                            <button onClick={() => setActiveMethod('QRIS')} className={`flex-1 py-2.5 rounded-xl font-black text-[10px] tracking-wider ${activeMethod === 'QRIS' ? 'bg-blue-600 text-white' : d ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-500'}`}>QRIS</button>
                            <button onClick={() => setActiveMethod('SPLIT')} className={`flex-1 py-2.5 rounded-xl font-black text-[10px] tracking-wider ${activeMethod === 'SPLIT' ? 'bg-purple-500 text-white' : d ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-500'}`}>SPLIT</button>
                        </div>

                        {activeMethod === 'SPLIT' && (
                            <div className="mb-4">
                                <label className={`text-[10px] font-black uppercase tracking-widest block mb-1 ${d ? 'text-zinc-400' : 'text-zinc-500'}`}>Nominal Cash yang dibayar</label>
                                <input type="number" value={splitCashInput} onChange={e => setSplitCashInput(e.target.value)}
                                    className={`w-full p-3 rounded-xl font-bold outline-none border text-xs mb-2 ${d ? 'bg-zinc-800 border-zinc-700 text-white' : 'bg-zinc-50 border-zinc-200 text-zinc-900'}`} placeholder="Masukkan nominal..." />
                                <p className={`text-[10px] font-bold ${d ? 'text-zinc-400' : 'text-zinc-500'}`}>
                                    Sisanya via QRIS: <span className="text-purple-500">Rp {(payModal.totalAmount - (parseInt(splitCashInput) || 0)).toLocaleString('id-ID')}</span>
                                </p>
                            </div>
                        )}

                        <button onClick={prosesLunasin} className="w-full bg-green-500 hover:bg-green-400 text-white font-black py-3.5 rounded-xl flex justify-center items-center gap-2 text-xs uppercase tracking-widest">
            ...
            <div className="flex-1 overflow-hidden flex flex-col min-w-0">
                <div className={`flex items-center justify-between px-5 md:px-8 py-4 border-b flex-shrink-0 ${d ? 'border-zinc-800 bg-zinc-900' : 'border-zinc-200 bg-white'}`}>
                    <div>
                        <h2 className="text-base font-black tracking-tight">Buku Hutang & Kasbon</h2>
                        <p className={`text-[9px] font-black uppercase tracking-widest ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>
                            {hutangList?.length ?? 0} pesanan belum dibayar
                        </p>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {(!hutangList || hutangList.length === 0) ? (
                            <div className={`col-span-full flex flex-col items-center justify-center py-20 ${d ? 'text-zinc-700' : 'text-zinc-300'}`}>
                                ...
                                <p className="font-black text-xs uppercase tracking-widest">Aman, nggak ada yang ngutang!</p>
                            </div>
                        ) : (
                            hutangList.map(h => (
                                <div key={h.id} className={`p-5 rounded-2xl border flex flex-col justify-between ${d ? 'bg-zinc-900 border-red-500/30' : 'bg-white border-red-200'}`}>
                                    <div>
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="flex gap-1.5 flex-wrap">
                                                <span className="text-[10px] font-black uppercase tracking-widest bg-red-500/10 text-red-500 px-2 py-1 rounded-md">BELUM BAYAR</span>
                                                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md border ${h.paymentMethod === 'QRIS' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' : h.paymentMethod === 'CASH' ? 'bg-green-500/10 text-green-600 border-green-500/20' : 'bg-zinc-500/10 text-zinc-500 border-zinc-500/20'}`}>
                                                    Rencana Via {h.paymentMethod === 'KASBON' ? 'CASH/QRIS' : h.paymentMethod}
                                                </span>
                                            </div>
                                            <span className={`text-[10px] font-bold ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>
                                                {h.date.toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}
                                            </span>
                                        </div>
                                        <h3 className="font-black text-lg mb-1 mt-2">{h.customerName}</h3>
                                        <p className="text-sm font-black text-amber-500 mb-4">Rp {h.totalAmount.toLocaleString('id-ID')}</p>

                                        <div className={`space-y-1 mb-5 p-3 rounded-lg ${d ? 'bg-zinc-800' : 'bg-zinc-50'}`}>
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => batalinHutang(h)} className={`w-1/3 border border-red-500/50 text-red-500 hover:bg-red-500/10 font-black py-2.5 rounded-xl transition-all active:scale-95 text-[10px] uppercase tracking-widest flex justify-center items-center gap-1`}>
                                            <Trash2 size={14} /> Batal
                                        </button>
                                        <button onClick={() => setPayModal(h)} className="w-2/3 bg-amber-500 hover:bg-amber-400 text-black font-black py-2.5 rounded-xl transition-all active:scale-95 text-[11px] uppercase tracking-widest flex justify-center items-center gap-1.5">
                                            <CheckCircle size={14} /> Lunasin
                                        </button>
                                    </div>
