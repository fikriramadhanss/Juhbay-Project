...
export default function HutangPage() {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('realtime-hutang-channel')
    ...
    const prosesLunasin = async () => {
        ...
        if (activeMethod === 'QRIS' || activeMethod === 'SPLIT') {
            if (!window.confirm(`PENTING: Saldo QRIS Rp ${qAmount.toLocaleString('id-ID')} beneran masuk?`)) return;
        }
        ...
        try {
            ...
            setStatusMessage(`Pembayaran kasbon a/n ${payModal.customerName} sukses dilunasi via ${activeMethod}.`);
            fetchHutangData();
        } catch (error) {
            setIsStatusModalOpen(true);
            setStatusType('ERROR');
            setStatusTitle('Gagal!');
            setStatusMessage('Terjadi kesalahan saat memproses pelunasan. Silakan coba lagi.');
        } finally {
            setPayModal(null);
            setSplitCashInput('');
        }
    ...
    const batalinHutang = async (h: any) => {
        if (!confirm(`Batalin kasbon a/n ${h.customerName}?`)) return;
        try {
            ...
            setIsStatusModalOpen(true);
            setStatusType('ERROR');
            setStatusTitle('Dibatalkan!');
            setStatusMessage(`Kasbon a/n ${h.customerName} berhasil dibatalkan. Stok menu telah dikembalikan.`);
            fetchHutangData();
        } catch (error) {
    ...
    if (!isMounted) {
        return (
            <div className="h-screen w-screen bg-zinc-950 flex items-center justify-center">
                <p className="text-amber-500 font-black tracking-[0.3em] uppercase text-xs">Memuat Buku Hutang...</p>
            </div>
        );
    }
    ...
    return (
        <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-300 ${d ? 'bg-zinc-950 text-white' : 'bg-zinc-100 text-zinc-900'}`}>
            ...
            {isStatusModalOpen && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="rounded-2xl p-8 md:p-10 w-full max-w-sm bg-zinc-900 border border-zinc-800 shadow-2xl text-center relative">
                        ...
                        <h3 className="text-white text-2xl font-black mb-1 md:mb-2">{statusTitle}</h3>
            ...
            {payModal && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className={`rounded-2xl p-5 md:p-6 w-full max-w-sm shadow-2xl border ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                        <div className="flex justify-between items-center mb-5"><h3 className="text-xs md:text-sm font-black uppercase tracking-widest text-amber-500">Pilih Pembayaran</h3><button onClick={() => setPayModal(null)} className={`p-1.5 rounded-lg ${d ? 'hover:bg-zinc-800' : 'hover:bg-zinc-100'}`}><X size={16} /></button></div>
                        ...
                        <div className="flex gap-2 mb-4">
                            <button onClick={() => setActiveMethod('CASH')} className={`flex-1 py-2 md:py-2.5 rounded-xl font-black text-[9px] md:text-[10px] ${activeMethod === 'CASH' ? 'bg-amber-500 text-black' : d ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-500'}`}>CASH</button>
                            <button onClick={() => setActiveMethod('QRIS')} className={`flex-1 py-2 md:py-2.5 rounded-xl font-black text-[9px] md:text-[10px] ${activeMethod === 'QRIS' ? 'bg-blue-600 text-white' : d ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-500'}`}>QRIS</button>
                            <button onClick={() => setActiveMethod('SPLIT')} className={`flex-1 py-2 md:py-2.5 rounded-xl font-black text-[9px] md:text-[10px] ${activeMethod === 'SPLIT' ? 'bg-purple-500 text-white' : d ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-500'}`}>SPLIT</button>
                        </div>
                        {activeMethod === 'SPLIT' && (
                            <div className="mb-4">
                                <input type="number" value={splitCashInput} onChange={e => setSplitCashInput(e.target.value)} className={`w-full p-2.5 rounded-xl font-bold outline-none border text-xs mb-2 ${d ? 'bg-zinc-800 border-zinc-700 text-white' : 'bg-zinc-50 border-zinc-200 text-zinc-900'}`} placeholder="Nominal Cash" />
                                <p className={`text-[9px] font-bold ${d ? 'text-zinc-400' : 'text-zinc-500'}`}>Sisa QRIS: <span className="text-purple-500">Rp {(payModal.totalAmount - (parseInt(splitCashInput) || 0)).toLocaleString('id-ID')}</span></p>
                            </div>
                        )}
                        <button onClick={prosesLunasin} className="w-full bg-green-500 hover:bg-green-400 text-white font-black py-3 md:py-3.5 rounded-xl flex justify-center items-center gap-2 text-[10px] md:text-xs uppercase tracking-widest"><CheckCircle size={14} /> Konfirmasi</button>
                    </div>
                </div>
            )}
            ...
            <div className={`w-14 md:w-56 flex flex-col p-2 md:p-4 gap-1 border-r z-20 flex-shrink-0 ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <div className="px-2 py-4 mb-2 hidden md:block"><p className="text-[9px] font-black text-amber-500 tracking-[0.3em] uppercase mb-1">EST. 2024</p><h1 className="text-base font-black tracking-tight leading-none">Juhbay POS</h1></div>
                <div className="flex justify-center md:hidden py-3"><Coffee size={20} className="text-amber-500" /></div>

                <Link href="/" className={`flex items-center justify-center md:justify-start gap-3 p-3 rounded-xl font-bold text-xs transition-all ${d ? 'text-zinc-500 hover:bg-zinc-800 hover:text-white' : 'text-zinc-400 hover:bg-zinc-100 hover:text-zinc-900'}`}><Store size={17} /> <span className="hidden md:block tracking-wide">Kasir</span></Link>
                <Link href="/history" className={`flex items-center justify-center md:justify-start gap-3 p-3 rounded-xl font-bold text-xs transition-all ${d ? 'text-zinc-500 hover:bg-zinc-800 hover:text-white' : 'text-zinc-400 hover:bg-zinc-100 hover:text-zinc-900'}`}><History size={17} /> <span className="hidden md:block tracking-wide">Riwayat</span></Link>
                <Link href="/manage" className={`flex items-center justify-center md:justify-start gap-3 p-3 rounded-xl font-bold text-xs transition-all ${d ? 'text-zinc-500 hover:bg-zinc-800 hover:text-white' : 'text-zinc-400 hover:bg-zinc-100 hover:text-zinc-900'}`}><Settings size={17} /> <span className="hidden md:block tracking-wide">Kelola Menu</span></Link>
            ...
            <div className="flex-1 overflow-hidden flex flex-col min-w-0">
                <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-8 py-4 border-b flex-shrink-0 gap-2 ${d ? 'border-zinc-800 bg-zinc-900' : 'border-zinc-200 bg-white'}`}>
                    <div>
                        ...
                        <p className={`text-[9px] font-black uppercase tracking-widest ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>{hutangList.length} pesanan belum dibayar</p>
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto p-4 md:p-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                        {hutangList.length === 0 ? (
                            <div className={`col-span-full flex flex-col items-center justify-center py-20 ${d ? 'text-zinc-700' : 'text-zinc-300'}`}><CheckCircle size={32} className="mb-3 md:w-10 md:h-10" /><p className="font-black text-[10px] md:text-xs uppercase tracking-widest text-center">Aman, nggak ada yang ngutang!</p></div>
                        ) : (
                            hutangList.map(h => (
                                    ...
                                    <div>
                                        <div className="flex justify-between items-start mb-2 md:mb-3">
                                            <div className="flex gap-1.5 flex-wrap">
                                                <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest bg-red-500/10 text-red-500 px-1.5 py-1 rounded-md">BELUM BAYAR</span>
                                        ...
                                        <div className={`space-y-2 mb-4 p-2 md:p-3 rounded-lg ${d ? 'bg-zinc-800' : 'bg-zinc-50'}`}>
                                            {h.items.map((item: any, idx: number) => {
                                                ...
                                                return (
                                                    <div key={idx} className="flex flex-col">
                                                        ...
                                                        <span className="text-[8px] text-green-500 font-bold mt-0.5">Hasil Bersih: Rp {profitPerItem.toLocaleString('id-ID')}</span>
                                                    </div>
                                                );
                                            })}
                                            <div className={`border-t ${d ? 'border-zinc-700' : 'border-zinc-300'} pt-1.5 mt-1.5 flex justify-between text-[9px] md:text-[10px] font-black`}>
                                                <span className={`${d ? 'text-zinc-500' : 'text-zinc-400'}`}>Total Hasil Bersih:</span>
                                                <span className="text-green-500">Rp {(h.totalProfit || 0).toLocaleString('id-ID')}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => batalinHutang(h)} className={`w-1/3 border border-red-500/50 text-red-500 hover:bg-red-500/10 font-black py-2 md:py-2.5 rounded-xl transition-all active:scale-95 text-[9px] md:text-[10px] uppercase tracking-widest flex justify-center items-center gap-1`}><Trash2 size={12} /> Batal</button>
                                        <button onClick={() => setPayModal(h)} className="w-2/3 bg-amber-500 hover:bg-amber-400 text-black font-black py-2 md:py-2.5 rounded-xl transition-all active:scale-95 text-[10px] md:text-[11px] uppercase tracking-widest flex justify-center items-center gap-1.5"><CheckCircle size={12} /> Lunasin</button>
                                    </div>
                                </div>
                            ))
                        )}
