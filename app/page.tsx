    ...
    <div className={`flex h-screen font-sans overflow-hidden relative transition-colors duration-300 ${d ? 'bg-zinc-950 text-white' : 'bg-zinc-100 text-zinc-900'}`}>
          ...
          <div className={`rounded-2xl p-5 md:p-8 w-full max-w-sm shadow-2xl border ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className={`p-2 rounded-lg ${d ? 'hover:bg-zinc-800' : 'hover:bg-zinc-100'}`}
              >
                <X size={18} />
              </button>
            </div>
            <div className="space-y-4">
              <div>
                <label className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest block mb-2 ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>
                  Nama Beans / Kopi
                </label>
                <input
                  type="text"
              ...
              <button
                ...
                className="w-full bg-amber-500 hover:bg-amber-400 text-black font-black py-3 md:py-4 rounded-xl transition-all active:scale-95 uppercase tracking-widest text-xs md:text-sm"
              >
                Masuk Keranjang
              </button>
            </div>
          ...
          <div className={`rounded-2xl p-5 md:p-8 w-full max-w-sm shadow-2xl border ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
            <div className="flex justify-between items-center mb-5 md:mb-6">
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${d ? 'hover:bg-zinc-800 text-zinc-400' : 'hover:bg-zinc-100 text-zinc-500'}`}
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="text-center mb-5 md:mb-6">
              <p className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-1 ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>
                Total Tagihan
              </p>
            ...
            <div className="mb-5 md:mb-6 text-left">
              <label className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest block mb-1 md:mb-2 ${d ? 'text-zinc-400' : 'text-zinc-500'}`}>
                Nama Pelanggan / Temen
              </label>
              <input
                type="text"
                value={customerName}
            ...
            <div className="space-y-2 md:space-y-3">
              <button
                ...
                className="w-full bg-amber-500 hover:bg-amber-400 text-black font-black py-3 md:py-4 rounded-xl transition-all active:scale-95 uppercase tracking-widest text-[10px] md:text-sm flex items-center justify-center gap-2"
              >
                <CheckCircle size={16} /> Udah Bayar Lunas
              </button>
              <button
                onClick={() => processPayment(true)}
                className={`w-full border-2 font-black py-2.5 md:py-3 rounded-xl transition-all active:scale-95 uppercase tracking-widest text-[9px] md:text-xs border-red-500/50 text-red-500 hover:bg-red-500/10`}
              >
                Belum Bayar (Ngutang)
              </button>
              <button
                onClick={cancelOrder}
                className={`w-full font-black py-2 rounded-xl transition-all active:scale-95 uppercase tracking-widest text-[9px] md:text-xs ${d ? 'text-zinc-500 hover:bg-zinc-800' : 'text-zinc-400 hover:bg-zinc-100'}`}
              >
                Kembali (Batal)
              </button>
            </div>
      ...
      <div className={`w-14 md:w-56 flex flex-col p-2 md:p-4 gap-1 border-r z-20 transition-colors duration-300 flex-shrink-0 ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
        ...
        <Link href="/" className="flex items-center justify-center md:justify-start gap-3 p-3 rounded-xl bg-amber-500 text-black font-black transition-all">
          <Store size={17} />
          <span className="hidden md:block text-xs tracking-wide">Kasir</span>
        </Link>
        ...
        <div className="mt-auto mb-2">
          <button
            ...
            className={`flex items-center justify-center md:justify-start gap-3 p-3 w-full rounded-xl font-black text-xs transition-all ${d ? 'bg-zinc-800 text-amber-500 hover:bg-amber-500 hover:text-black' : 'bg-zinc-100 text-amber-600 hover:bg-amber-500 hover:text-white'}`}
          >
            <FileText size={17} />
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 min-h-0">
          <h2 className={`text-xs font-black tracking-[0.3em] uppercase mb-4 md:mb-5 ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>
            Order Menu
          </h2>

          {activeProducts && Object.entries(activeProducts).map(([cat, names]) => (
            <div key={cat} className="mb-5 md:mb-7">
              <h3 className={`text-[8px] md:text-[9px] font-black tracking-[0.3em] uppercase mb-2 md:mb-3 px-3 py-1 rounded-full inline-block border ${d ? 'text-amber-500 bg-amber-500/10 border-amber-500/20' : 'text-amber-700 bg-amber-50 border-amber-200'}`}>
                {cat}
              </h3>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-3">
                {Object.entries(names).map(([name, variants]) => (
                  <div
                    key={name}
                    className={`p-3 md:p-4 rounded-2xl border flex flex-col justify-between transition-all ${d ? 'bg-zinc-900 border-zinc-800 hover:border-zinc-700' : 'bg-white border-zinc-200 shadow-sm hover:border-zinc-300'}`}
                  >
                    <h4 className="font-black text-[11px] md:text-sm mb-2 md:mb-3 leading-tight">
                      {name}
                    </h4>
                      ...
                      {variants.map(v => (
                        ...
                        >
                          <div className="flex flex-col items-start gap-0 md:gap-0.5">
                            <span className="opacity-50 uppercase text-[7px] md:text-[8px] tracking-wider">
                              {v.variant || 'Regular'}
                            </span>
                            <span className={`text-[7px] md:text-[8px] font-bold group-hover:text-black/60 ${d ? 'text-amber-500' : 'text-amber-600'}`}>
                              Sisa: {v.stock}
                            </span>
                          </div>
                          <span className="text-[9px] md:text-[10px] font-black">
                            {v.price === 0 ? 'PILIH' : `Rp ${v.price.toLocaleString('id-ID')}`}
                          </span>
                        </button>
                      ))}
                    </div>
        ...
        <div className={`w-full md:w-72 lg:w-80 flex flex-col border-t md:border-t-0 md:border-l flex-shrink-0 transition-colors duration-300 h-[45vh] md:h-auto ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
          ...
          <div className={`p-3 md:p-5 border-b flex justify-between items-center flex-shrink-0 ${d ? 'border-zinc-800' : 'border-zinc-100'}`}>
            <h2 className="font-black flex items-center gap-2 text-xs md:text-sm">
              ...
              Keranjang
              {cart.length > 0 && (
                <span className="bg-amber-500 text-black text-[8px] md:text-[9px] font-black w-4 h-4 md:w-4 md:h-4 rounded-full flex items-center justify-center ml-1">
                  {cart.length}
                </span>
              )}
            </h2>
          ...
          <div className="flex-1 p-2 md:p-3 overflow-y-auto space-y-1.5 md:space-y-2 min-h-0">
            ...
            {cart.map(item => (
              <div
                key={item.cartKey}
                className={`flex justify-between items-center p-2 md:p-3 rounded-xl border ${d ? 'bg-zinc-800 border-zinc-700' : 'bg-zinc-50 border-zinc-200'}`}
              >
                <div className="flex-1 min-w-0 pr-2">
                  <p className="text-[9px] md:text-[11px] font-black truncate leading-tight">
                    {item.name}
                  </p>
                  <p className={`text-[8px] md:text-[10px] font-bold mt-0.5 ${d ? 'text-amber-500' : 'text-amber-600'}`}>
                    Rp {item.price.toLocaleString()}
                    <span className={`font-normal ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>
                      x{item.quantity}
                    </span>
                  </p>
                </div>
          ...
          <div className={`p-3 md:p-5 border-t flex-shrink-0 ${d ? 'border-zinc-800 bg-zinc-950' : 'border-zinc-100 bg-zinc-50'}`}>
            <div className="mb-2 md:mb-3">
              <p className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest mb-1.5 md:mb-2 ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>
                Metode Bayar (Lunas)
              </p>
            ...
            {paymentMethod === 'SPLIT' && (
              <div className={`p-2 md:p-3 rounded-xl border mb-2 md:mb-3 ${d ? 'bg-zinc-900 border-zinc-700' : 'bg-zinc-50 border-zinc-200'}`}>
                <label className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest block mb-1 ${d ? 'text-zinc-400' : 'text-zinc-500'}`}>
                  Nominal Cash
                </label>
                <input
                  type="number"
                  ...
                  className={`w-full p-1.5 md:p-2 rounded-lg font-bold outline-none border text-[10px] md:text-xs mb-1.5 md:mb-2 ${d ? 'bg-zinc-800 border-zinc-700 text-white' : 'bg-white border-zinc-200'}`}
            ...
            <div className="flex justify-between items-end mb-2 md:mb-3 mt-1">
              <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>
                Total
              </span>
