            ...
            <div className="flex-1 overflow-y-auto p-5 md:p-8">
                <div className="max-w-6xl mx-auto">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                        <div>
                            <h2 className="text-3xl font-black tracking-tight mb-1">History</h2>
                            <p className={`text-xs font-black uppercase tracking-widest ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>Laporan Penjualan</p>
                        </div>
                        <div className="flex flex-wrap items-center gap-3">
                            <input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className={`p-2.5 rounded-xl font-bold text-xs outline-none border ${d ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-zinc-200 text-zinc-900'}`} />
                            <button onClick={handleExportPDF} className="bg-amber-500 hover:bg-amber-400 text-black font-black px-4 py-2.5 rounded-xl transition-all active:scale-95 text-xs uppercase tracking-widest flex items-center gap-2"><Download size={14} /> PDF</button>
                            <button onClick={handleClearHistory} className={`border border-red-500/50 text-red-500 hover:bg-red-500/10 font-black px-4 py-2.5 rounded-xl transition-all active:scale-95 text-xs uppercase tracking-widest flex items-center gap-2`}><Trash2 size={14} /> Hapus</button>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                        <div className={`p-5 rounded-2xl border ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                            <p className={`text-[10px] font-black uppercase tracking-widest mb-2 ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>Transaksi</p>
                            <h3 className="text-2xl font-black">{historyData?.totalTrx || 0}</h3>
                        </div>
                        <div className={`p-5 rounded-2xl border ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                            <p className={`text-[10px] font-black uppercase tracking-widest mb-2 ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>Total</p>
                            <h3 className="text-2xl font-black">Rp {historyData?.totalAmount.toLocaleString('id-ID') || 0}</h3>
                        </div>
                        <div className={`p-5 rounded-2xl border ${d ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                            <p className={`text-[10px] font-black uppercase tracking-widest mb-2 ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>Mutasi QRIS</p>
                            <h3 className="text-2xl font-black">{historyData?.qrisCount || 0} <span className="text-sm font-bold text-zinc-500">trx</span></h3>
                        </div>
                    </div>
                            ...
                            <table className="w-full text-left min-w-[700px]">
                                <thead className={`border-b ${d ? 'border-zinc-800' : 'border-zinc-200'}`}>
                                    <tr className={`text-[9px] font-black uppercase tracking-widest ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>
                                        <th className="px-6 py-4">ID</th><th className="px-6 py-4">Waktu</th><th className="px-6 py-4">Nama Pembeli</th>
                                        <th className="px-6 py-4">Metode</th><th className="px-6 py-4">Items</th><th className="px-6 py-4 text-right">Total</th>
                                    </tr>
                                </thead>
                                <tbody className={`divide-y ${d ? 'divide-zinc-800/50' : 'divide-zinc-100'}`}>
                                    {(!historyData?.sales || historyData.sales.length === 0) ? (
                                        <tr><td colSpan={6} className="px-6 py-12 text-center"><p className={`text-xs font-black uppercase tracking-widest ${d ? 'text-zinc-600' : 'text-zinc-400'}`}>Belum ada transaksi</p></td></tr>
                                    ) : (
                                        historyData.sales.map((sale) => {
                                            ...
                                            return (
                                                <tr key={sale.id} className={`transition-colors ${d ? 'hover:bg-zinc-800/50' : 'hover:bg-zinc-50'}`}>
                                                    <td className="px-6 py-4"><span className="text-[11px] font-black text-amber-500">#TRX-{sale.id}</span></td>
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col"><span className={`text-[10px] font-bold ${d ? 'text-zinc-300' : 'text-zinc-700'}`}>{tgl}</span><span className={`text-[9px] font-black ${d ? 'text-zinc-500' : 'text-zinc-400'}`}>{jam}</span></div>
                                                    </td>
                                                    <td className="px-6 py-4"><span className={`text-xs font-bold ${d ? 'text-zinc-300' : 'text-zinc-700'}`}>{sale.customerName && sale.customerName !== '-' ? sale.customerName : <span className="opacity-50 italic">Anonim</span>}</span></td>
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col items-start gap-1">
                                                            ...
                                                            {sale.paymentMethod === 'SPLIT' && (
                                                                <div className={`text-[8px] font-bold mt-1 leading-tight ${d ? 'text-zinc-400' : 'text-zinc-500'}`}>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="space-y-1">{sale.items.map((item, idx) => (<div key={idx} className={`text-[11px] font-bold ${d ? 'text-zinc-300' : 'text-zinc-700'}`}>{item.name} <span className="text-amber-500 ml-1">x{item.quantity}</span></div>))}</div>
                                                    </td>
                                                    <td className="px-6 py-4 text-right"><span className="text-sm font-black">Rp {sale.totalAmount.toLocaleString('id-ID')}</span></td>
                                                </tr>
                                            );
                                        })
                                    )}
